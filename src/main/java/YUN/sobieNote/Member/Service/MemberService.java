...
public class MemberService {
    ...
    public Member findOrSaveMember(MemberLoginRequest memberLoginRequest, int authProviderId, int memberRoleId) {

        AuthProvider authProvider = authProviderRepository.findById(authProviderId)
                .orElseThrow(() -> new IllegalArgumentException("해당 소셜 로그인 방식이 없습니다. auth_provider_id: " + authProviderId));

        MemberRole memberRole = memberRoleRepository.findById(memberRoleId)
                .orElseThrow(() -> new IllegalArgumentException("해당 유저 권한을 찾을 수 없습니다. member_role_id: " + memberRoleId));


        return memberRepository.findByEmail(memberLoginRequest.getEmail())
                .orElseGet(() -> {
                    Member newMember = new Member(memberLoginRequest,authProvider,memberRole);
